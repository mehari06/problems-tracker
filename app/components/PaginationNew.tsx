...
const Pagination = ({ itemCount, pageSize, currentPage }: Props): React.ReactElement | null => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const pathname = usePathname();
  ...
  const safeCurrentPage = Number.isNaN(currentPage) 
    ? 1 
  ...
  const changePage = (page: number): void => {
    const params = new URLSearchParams(searchParams?.toString() ?? '');
    params.set('page', page.toString());
    // Remove pageSize from URL params to avoid conflicts
    // params.set('pageSize', pageSize.toString());
    
    const url = `${pathname}?${params.toString()}`;
    router.push(url);
    router.refresh(); // ← CRITICAL: This refreshes server components
  };
